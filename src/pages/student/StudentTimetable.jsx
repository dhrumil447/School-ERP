...
export default function StudentTimetable() {
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 3 } }}>
      <Box sx={{ mb: 3 }}>
        <Typography variant="h5" sx={{ fontWeight: 800, mb: 0.5 }}>My Timetable</Typography>
        <Typography sx={{ color: 'text.secondary', fontSize: '0.875rem' }}>Weekly class schedule for Class 10A</Typography>
      </Box>
            ...
            {mockDays.map((day, di) => (
              <Box key={day} sx={{ display: 'grid', gridTemplateColumns: '90px repeat(7, 1fr)', borderBottom: '1px solid', borderColor: 'divider', bgcolor: di % 2 === 0 ? 'transparent' : 'action.hover' }}>
                <Box sx={{ p: 1.5, display: 'flex', alignItems: 'center' }}>
                  <Typography sx={{ fontWeight: 700, fontSize: '0.8rem', color: 'primary.main' }}>{day.slice(0, 3)}</Typography>
                </Box>
                {(schedule[day] || []).map((subject, pi) => {
                  ...
                  return (
                    <Box key={pi} sx={{ p: 0.75, borderLeft: '1px solid', borderColor: 'divider', textAlign: 'center' }}>
                      <Box sx={{ py: 0.8, borderRadius: 1.5, bgcolor: `${color}18`, border: `1.5px solid ${color}40` }}>
                        <Typography sx={{ fontSize: '0.7rem', fontWeight: 700, color }}>{subject}</Typography>
                      </Box>
                    </Box>
                  );
                })}
